    ...
    <div className="h-full flex flex-col bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 selection:bg-zinc-900 selection:text-white dark:selection:bg-white dark:selection:text-black overflow-hidden relative transition-colors duration-300">
      ...
      <nav className="sticky top-0 z-50 bg-white/70 dark:bg-zinc-950/70 backdrop-blur-xl border-b border-zinc-200/60 dark:border-zinc-800/60 supports-[backdrop-filter]:bg-white/60">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <button onClick={() => onNavigate('home')} className="flex items-center gap-3 group relative z-10 outline-none">
            {/* Abstract Logo */}
            ...
            <span className="font-display font-bold text-xl tracking-tight text-zinc-900 dark:text-white">Stats<span className="text-zinc-400 dark:text-zinc-600 font-normal">Analysis</span></span>
          </button>
          ...
          <div className="flex items-center gap-4 relative z-10">
             ...
             <button 
                ...
                className="btn-lyra-primary text-sm px-6 py-2.5"
             >
                Get Started
             </button>
          </div>
        ...
        <section className="py-24 border-t border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-sm">
          <div className="max-w-7xl mx-auto px-6">
             <div className="grid md:grid-cols-2 gap-16 mb-16">
               <div>
                  <h2 className="text-2xl font-bold mb-6 tracking-tight font-display text-zinc-900 dark:text-white">Contact Us</h2>
                  <div className="space-y-4 text-zinc-500 dark:text-zinc-400 text-sm">
                    <p className="max-w-md leading-relaxed">
                      Ready to start? Contact us for a free initial consultation. We ensure your research is built on a solid statistical foundation.
                    </p>
                    <div className="p-1 pr-4 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-full w-fit flex items-center gap-3 pl-3 py-2">
                      ...
                      <span className="font-mono text-zinc-600 dark:text-zinc-300 font-medium">analysis@example.com</span>
                    </div>
                  </div>
               </div>
               <div className="grid grid-cols-2 gap-12 text-sm text-zinc-500 dark:text-zinc-400">
                  <div>
                    <strong className="text-zinc-900 dark:text-white block mb-6 font-display text-base">Pages</strong>
                    <ul className="space-y-3">
                      <li><button onClick={() => onNavigate('home')} className="hover:text-black dark:hover:text-white hover:translate-x-1 transition-transform inline-block">Home</button></li>
                      <li><button onClick={() => onNavigate('services')} className="hover:text-black dark:hover:text-white hover:translate-x-1 transition-transform inline-block">Services</button></li>
                      <li><button onClick={() => onNavigate('about')} className="hover:text-black dark:hover:text-white hover:translate-x-1 transition-transform inline-block">About</button></li>
                    </ul>
                  </div>
                  <div>
                     <strong className="text-zinc-900 dark:text-white block mb-6 font-display text-base">Legal</strong>
                     <ul className="space-y-3">
                      <li className="hover:text-black dark:hover:text-white cursor-pointer hover:translate-x-1 transition-transform inline-block">Privacy Policy</li>
                      <li className="hover:text-black dark:hover:text-white cursor-pointer hover:translate-x-1 transition-transform inline-block">Terms of Service</li>
                     </ul>
                  </div>
               </div>
             </div>
